...
# Faker library allows us to create fake data
...
app = Flask(__name__)
# sqlite:///    --  represent file in current directory
app.config['SQLALCHEMY_DATABASE_URI'] = 'sqlite:///db.sqlite3'
# SQLALCHEMY_TRACK_MODIFICATIONS  ---  just to suppress some warnings
...
# a customer can have any orders but an order have only one customer ( one to many relationship)
...
# association table
# order_product will show that order_id x is association with product_id y
...
#  for one order we have many products
#  We can have multiple orders associated with one product
#  so order and products have many to many relationship
...
class Order(db.Model):
    ...
    # products will take a list of Product_objects
...
def add_customers():
    for _ in range(100):
        customer = Customer(
            # we are adding fake data of customer
...
def add_orders():
    ...
    for _ in range(1000):
        ...
        coupon_code = random.choices(
            [None, '50OFF', 'FREESHIPPING', 'BUYONEGETONE'], [80, 5, 5, 5])[0]
...
def add_order_products():
    ...
    for order in orders:
        # select random k (# of products in a perticular order)
        k = random.randint(1, 3)
        # select random products
        # random.sample will return k # of products
...
def create_random_data():  # create sqlite file with random data
    db.create_all()
    add_customers()
    add_orders()
    add_products()
...
def get_orders_by(customer_id=1):
    ...
    # show all the orders of the customer with that customer_id
...
# python
# from app import how_many_customers
# how_many_customers()
...
# db.session.query(Order)  = Order.query....
...
def get_customers_who_have_purchased_x_dollars(amount=500):
    ...
    customers = db.session.query(Customer).join(Order).join(order_product).join(
        Product).group_by(Customer).having(db.func.sum(Product.price) > amount).all()
